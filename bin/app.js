...
class CDKProject {
  ...
  /**
   * Gets the sandbox name from the CDK context if provided.
   * Sandbox names allow creating isolated personal development environments.
   *
   * @returns {string|null} The sandbox name or null if not a sandbox deployment
   */
  getSandboxName() {
  ...
  /**
   * Checks if this is a sandbox deployment.
   *
   * @returns {boolean} True if a sandboxName context parameter is provided
   */
  isSandbox() {
  ...
  async createStacks() {
    ...
    dep(publicApiStack, referenceDataStack);
    dep(publicApiStack, bookingWorkflowStack);
    dep(publicApiStack, emailDispatchStack);
    dep(publicApiStack, waitingRoomStack);
    dep(adminApiStack, referenceDataStack);
    dep(adminApiStack, waitingRoomStack);
    dep(openSearchStack, adminIdentityStack);
    dep(openSearchStack, publicIdentityStack);
    dep(bookingWorkflowStack, coreStack);
    dep(bookingWorkflowStack, transactionalDataStack);
    dep(bookingWorkflowStack, referenceDataStack);
    dep(emailDispatchStack, coreStack);
    dep(referenceDataStack, coreStack);
    dep(referenceDataStack, openSearchStack);
    dep(transactionalDataStack, coreStack);
