...
const updateConfig = async (attributes, validAttributes, sk) => {
    ...
    console.log('Update config params:', params);
    try {
...
const parseArguments = () => {
    ...
    try {
        ...
        const rawAttributes = cfnOutput[stackName]; // Access attributes using stackName
        const validAttributesTemplate = JSON.parse(fs.readFileSync(args[1], 'utf8'));
        
        // Get environment from process.env.ENVIRONMENT
        const environment = process.env.ENVIRONMENT;
        
        // Replace ${ENVIRONMENT} placeholders in valid attributes with actual environment
        const validAttributes = validAttributesTemplate.map(attr => 
            attr.replace('${ENVIRONMENT}', environment)
        );
        
        return { attributes: rawAttributes, validAttributes, sk };
    } catch (e) {
