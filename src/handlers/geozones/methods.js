...
/**
 * Retrieves all geozones matching an CollectionId.
 *
 * @async
 * @param {Object} collectionId - The CollectionId of the geozone.
 ...
 *
 */
async function getGeozonesByCollectionId(
  collectionId,
  filters,
  params = null
) {
  logger.info("Get Geozones By CollectionId");
  try {
 ...
 *
 * @param {Object} collectionId - collectionId (e.g. bcparks::1)
 ...
 */
async function parseRequest(collectionId, body, requestType, geozoneId = null) {
  let updateRequests = [];
  // Check if the request is a batch request
  if (Array.isArray(body)) {
    for (let item of body) {
      updateRequests.push(
        await processItem(collectionId, item, requestType, geozoneId)
      );
    }
  } else {
    updateRequests.push(
      await processItem(collectionId, body, requestType, geozoneId)
    );
  }
