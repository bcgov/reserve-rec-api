...
const defaults = {
  ...
  constructs: {
    ...
    entityRelationshipGsi: {
      name: 'entityRelationship-index',
    },
    dynamoStreamRole: {
  ...
  config: {
    ...
    userIdGsiSortKey: 'sk',
    entityRelationshipGsiPartitionKey: 'gsipk',
    entityRelationshipGsiSortKey: 'gsisk',
  ...
  constructor(scope, primer) {
    ...
    if (this.overrides?.referenceDataTableName) {
      ...
      this.refDataTable = new dynamodb.Table(this, this.getConstructId('referenceDataTable'), {
        ...
        pointInTimeRecoverySpecification: { pointInTimeRecoveryEnabled: !this.isSandbox() },
        billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
        deletionProtection: !this.isSandbox(),
        removalPolicy: this.isSandbox() ? RemovalPolicy.DESTROY : RemovalPolicy.RETAIN,
    ...
    // Add Global Secondary Index (Entity Relationship)
    this.refDataTable.addGlobalSecondaryIndex({
      indexName: this.getConstructName('entityRelationshipGsi'),
      partitionKey: { name: this.getConfigValue('entityRelationshipGsiPartitionKey'), type: dynamodb.AttributeType.STRING },
      sortKey: { name: this.getConfigValue('entityRelationshipGsiSortKey'), type: dynamodb.AttributeType.STRING },
      projectionType: dynamodb.ProjectionType.ALL
    });
