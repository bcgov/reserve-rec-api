...
class EmailDispatchStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Store primer for access to env properties
    this.primer = primer;
    ...
    // Get Admin Frontend Domain, used to build the QR code verification URL
    // so scanning it opens Admin's Sales screen (bcgov/reserve-rec-admin#335)
    ...
    // Create Email Dispatch Lambda Function using NodejsFunction for esbuild bundling
    // This allows dependencies from the base layer (like handlebars) to be used instead of bundling them
    this.emailDispatchFunction = new NodejsFunction(this, this.getConstructId('emailDispatchFunction'), {
      ...
      entry: path.join(__dirname, '../handlers/emailDispatch/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          'aws-sdk',      // Exclude AWS SDK v2
          '@aws-sdk/*',   // Exclude AWS SDK v3
          '/opt/*',       // Exclude Lambda layer dependencies (handlebars, etc.)
        ],
        minify: false,    // Easier debugging in CloudWatch
        // esbuild bundles the JS graph only, so the brand PNGs the confirmation
        // template references by Content-ID never reached the deployed bundle:
        // the handler logged "Brand image missing" and sent the email without a
        // logo or icons. Copy them next to the bundled index.js, where
        // __dirname resolves at runtime (bcgov/reserve-rec-public#731).
        commandHooks: {
          beforeBundling: () => [],
          beforeInstall: () => [],
          afterBundling: (inputDir, outputDir) => [
            `mkdir -p ${outputDir}/assets`,
            `cp ${inputDir}/lib/handlers/emailDispatch/assets/*.png ${outputDir}/assets/`,
          ],
        },
      },
      environment: {
        ...
        ADMIN_ALTERED_FRONTEND_DOMAIN: adminFrontendDomain,
