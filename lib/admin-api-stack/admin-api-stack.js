...
const { AdminTransactionsConstruct }     = require('../../src/handlers/transactions/constructs');
...
const { AdminTransactionsNestedStack }   = require('./admin-transactions-nested-stack/admin-transactions-nested-stack');
...
const { AdminCollectionsNestedStack }    = require('./admin-collections-nested-stack/admin-collections-nested-stack');
...
const defaults = {
  ...
  constructs: {
    ...
    inventoryConstruct:             { name: "InventoryConstruct" },
    inventoryPoolsConstruct:        { name: "InventoryPoolsConstruct" },
    productDatesConstruct:          { name: "ProductDatesConstruct" },
    usersConstruct:                 { name: "UsersConstruct" },
    verifyConstruct:                { name: "VerifyConstruct" },
    collectionsConstruct:           { name: "CollectionsConstruct" },
    adminTransactionsConstruct:     { name: "AdminTransactionsConstruct" },
    ...
    adminTransactionsNestedStack:   { name: "AdminTransactionsNestedStack" },
    ...
    collectionsNestedStack:         { name: "CollectionsNestedStack" },
  ...
  config: {
    ...
    enableAdminTransactions: process.env.ENABLE_ADMIN_TRANSACTIONS !== 'false',
...
class AdminApiStack extends BaseStack {
  constructor(scope, primer) {
    ...
    if (this.getConfigValue("enableCollections")) {
      ...
      if (isLocal) {
        this.collectionsConstruct = new AdminCollectionsConstruct(
          this,
          this.getConstructId("collectionsConstruct"),
          collectionsProps,
        );
      } else {
        this.collectionsNestedStack = new AdminCollectionsNestedStack(
          this,
    ...
    if (this.getConfigValue("enableProductManagement")) {
      ...
      if (isLocal) {
        logger.info(
          "Local context detected - using flat ProductDates/Inventory/InventoryPools constructs for SAM compatibility",
        );
        ...
        this.inventoryConstruct = new InventoryConstruct(
          this,
          this.getConstructId("inventoryConstruct"),
          productManagementProps,
        );
        this.inventoryPoolsConstruct = new InventoryPoolsConstruct(
          this,
          this.getConstructId("inventoryPoolsConstruct"),
          productManagementProps,
        );
      } else {
        // For AWS deployments, use the nested stack to reduce resource count
    ...
    // Admin Transactions Lambdas
    if (this.getConfigValue('enableAdminTransactions')) {
      const transactionsProps = {
        environment: {
          LOG_LEVEL: this.getConfigValue('logLevel'),
          TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
          MERCHANT_ID_SECRET: `/reserveRecApi/${this.getDeploymentName()}/adminApiStack/merchantId`,
          HASH_KEY_SECRET: `/reserveRecApi/${this.getDeploymentName()}/adminApiStack/hashKey`,
          WORLDLINE_PAYMENT_API_URL: 'https://api.na.bambora.com/v1/payments',
          PAYMENTS_API_SECRET: `/reserveRecApi/${this.getDeploymentName()}/adminApiStack/paymentsApiPasscode`
        },
        layers: [baseLayer, awsUtilsLayer],
      ...
      // For local, use the flat construct for SAM compatibility.
      if (isLocal) {
        this.adminTransactionsConstruct = new AdminTransactionsConstruct(
          this,
          this.getConstructId('adminTransactionsConstruct'),
          transactionsProps,
        );
      } else {
        // For AWS deployments, use the nested stack to reduce resource count
        // in the root stack
        this.adminTransactionsNestedStack = new AdminTransactionsNestedStack(
          this,
          this.getConstructId('adminTransactionsNestedStack'),
          transactionsProps,
        );
        this.adminTransactionsConstruct = this.adminTransactionsNestedStack.adminTransactionsConstruct;
      }
    }
    ...
    if (this.getConfigValue('enableWaitingRoom')) {
      this.waitingRoomAdminNestedStack = new AdminWaitingRoomNestedStack(this, this.getConstructId('waitingRoomAdminNestedStack'), {
        ...
        frontDoorViewerFunctionName: scope.resolveFrontDoorViewerFnName(this),
        wsManagementEndpoint: scope.resolveWsManagementEndpoint ? scope.resolveWsManagementEndpoint() : '',
        webSocketApiId: scope.resolveWebSocketApiId ? scope.resolveWebSocketApiId() : '',
      });
    }
    ...
    const nestedStacks = [
      ...
      isLocal ? null : this.adminTransactionsNestedStack,
    ...
    const adminAccessLogs = new logs.LogGroup(this, 'AdminApiAccessLogs', {
      logGroupName: `/aws/apigateway/${this.getAppName()}-${this.getDeploymentName()}-admin-access`,
    ...
    new apigw.Stage(this, "AdminApiStage", {
      ...
      // Admin traffic carries staff and visitor detail; payload tracing put all
      // of it in CloudWatch. The access log keeps the shape without the bodies.
      dataTraceEnabled: false,
      metricsEnabled: true,
      accessLogDestination: new apigw.LogGroupLogDestination(adminAccessLogs),
