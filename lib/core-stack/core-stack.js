...
const defaults = {
  ...
  config: {
    ...
    adminAlteredDomainNameSSMPath: null,
    emailQueueUrlSSMPath: null,
    // No dynamic fallback — set in SSM config only for environments with a deployed front door.
    frontDoorViewerFnNameSSMPath: null,
  }
};
...
class CoreStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Default the dynamic SSM paths from the deployment environment, but let the
    // stack's SSM config override them — these previously clobbered any configured
    // value, which made it impossible to repoint (e.g. publicDomainNameSSMPath at
    // the front door's publicFrontendBase export, reserve-rec-api#207).
    const deploymentName = scope.getDeploymentName();
    if (!this.getConfigValue('publicDomainNameSSMPath')) {
      this.setConfigValue('publicDomainNameSSMPath', `/reserveRecPublic/${deploymentName}/distributionStack/distributionDomainName`);
    }
    if (!this.getConfigValue('adminDomainNameSSMPath')) {
      this.setConfigValue('adminDomainNameSSMPath', `/reserveRecAdmin/${deploymentName}/distributionStack/distributionDomainName`);
    }
    // adminFrontendBase is the vanity host when the admin distribution has one,
    // else its *.cloudfront.net name. It is the only admin host export that exists
    // in every environment (bcgov/reserve-rec-public#854).
    if (!this.getConfigValue('adminAlteredDomainNameSSMPath')) {
      this.setConfigValue('adminAlteredDomainNameSSMPath', `/reserveRecAdmin/${deploymentName}/distributionStack/adminFrontendBase`);
    }
    if (!this.getConfigValue('emailQueueUrlSSMPath')) {
      this.setConfigValue('emailQueueUrlSSMPath', `reserveRecAdmin/${deploymentName}/emailDispatchStack/emailQueueUrl`)
    }
    if (!this.getConfigValue('waitingRoomViewerFnNameSSMPath')) {
    ...
    scope.resolveAdminAlteredDomainName = this.resolveAdminAlteredDomainName.bind(this);
    ...
    scope.resolveFrontDoorViewerFnName = this.resolveFrontDoorViewerFnName.bind(this);
  ...
  // Get the altered domain (vanity url) for the admin site from SSM
  resolveAdminAlteredDomainName(consumerScope) {
    return this.resolveReference(consumerScope, this.getConfigValue('adminAlteredDomainNameSSMPath'));
  }
  ...
  resolveEmailQueueUrl(consumerScope) {
    return this.resolveReference(consumerScope, this.getConfigValue('emailQueueUrlSSMPath'));
  }
  ...
  // Gets the front door's /dayuse* viewer fn name (SPA fallback + Mode 2 gate target).
  // Only resolves when frontDoorViewerFnNameSSMPath is configured — environments without
  // a deployed front door return '' so the toggle Lambda skips that target.
  resolveFrontDoorViewerFnName(consumerScope) {
    const ssmPath = this.getConfigValue('frontDoorViewerFnNameSSMPath');
    return ssmPath ? this.resolveReference(consumerScope, ssmPath) : '';
  }
