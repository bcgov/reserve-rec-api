...
class StackPrimer {
  ...
  nameConstructs() {
    for (const [key, construct] of Object.entries(this.constructs)) {
      if (construct.hasOwnProperty('name')) {
        ...
        construct.cfnOutput = this.createConstructCfnOutput(key);
      } else {
  ...
  async getDeploymentConfig() {
    try {
      if (this.getDeploymentName() === 'local' || this.isOffline()) {
        ...
        const localContext = require('../lib/env.json');
        if (!localContext?.stackContexts?.[this.stackKey]) {
          logger.warn(`Local context for stack ${this.stackKey} not found in env.json`);
        } else {
          this.config = Object.assign(localContext?.stackContexts?.[this.stackKey], this.config);
        }
      } else {
  ...
  createConstructCfnOutput(constructKey, delimiter = '-') {
  ...
  createConstructSSMPath(constructKey) {
    return '/' + this.createConstructCfnOutput(constructKey, '/');
  }
