const { BaseStack } = require('../helpers/base-stack');
const { Duration } = require('aws-cdk-lib');
const lambda = require('aws-cdk-lib/aws-lambda');
const iam = require('aws-cdk-lib/aws-iam');
const { SqsSubscription } = require('aws-cdk-lib/aws-sns-subscriptions');
const { logger } = require("../helpers/utils");
const { NodejsFunction } = require('aws-cdk-lib/aws-lambda-nodejs');
const { Queue } = require('aws-cdk-lib/aws-sqs');
const { SqsEventSource } = require('aws-cdk-lib/aws-lambda-event-sources');
const { StackPrimer } = require("../helpers/stack-primer");
const { Topic } = require('aws-cdk-lib/aws-sns');
const path = require('path');
const targets = require('aws-cdk-lib/aws-events-targets');
const events = require('aws-cdk-lib/aws-events');
const cloudwatch = require('aws-cdk-lib/aws-cloudwatch');
const { eventMetricNamespace } = require('../helpers/event-metrics');

const defaults = {
  constructs: {
    refundRequestTopic: {
      name: 'RefundRequestTopic',
    },
    worldlineDeadLetterQueue: {
      name: 'WorldlineDeadLetterQueue',
    },
    worldlineRequestQueue: {
      name: 'WorldlineRequestQueue',
    },
    refundSubscriber: {
      name: 'RefundSubscriber',
    },
    worldlineProcessor: {
      name: 'WorldlineProcessor',
    },
    refundSubscriberDLQ: {
      name: 'RefundSubscriberDLQ',
    },
    refundRequestQueue: {
      name: 'RefundRequestQueue',
    },
    inventoryReturnDeadLetterQueue: {
      name: 'InventoryReturnDeadLetterQueue',
    },
    inventoryReturnDeadLetterAlarm: {
      name: 'InventoryReturnDeadLetterAlarm',
    },
    inventoryReturnQueue: {
      name: 'InventoryReturnQueue',
    },
    smsReminderDeadLetterQueue: {
      name: 'SmsReminderDeadLetterQueue',
    },
    smsReminderQueue: {
      name: 'SmsReminderQueue',
    },
    smsReminderProcessor: {
      name: 'SmsReminderProcessor',
    },
    inventoryReturnProcessor: {
      name: 'InventoryReturnProcessor',
    },
    inventoryMetricsFn: {
      name: 'InventoryMetrics',
    },
    inventoryMetricsSchedule: {
      name: 'InventoryMetricsSchedule',
    },
    expiredBookingCleanupFn: {
      name: 'ExpiredBookingCleanupFn',
    },
    incompleteBookingScraperSchedule: {
      name: 'IncompleteBookingScraperSchedule',
    },
  },
  config: {
    logLevel: process.env.LOG_LEVEL || 'info',
    // Topic configuration - note: actual names will be prefixed with app name and deployment name
    refundRequestTopicBaseName: 'refund-requests',
    // Subscription configuration
    enableEmailSubscriptions: false,
    // Topic retention and policies
    topicRemovalPolicy: 'retain',
    enableTopicEncryption: true,
    // Refund workflow configuration
    enableRefundWorkflow: true,
    refundDispatchBatchSize: 1, // process one refund at a time
    refundDispatchMaxBatchingWindow: 5, // seconds
    worldlineDispatchBatchSize: 1, // process one Worldline request at a time
    worldlineDispatchMaxBatchingWindow: 5, // seconds
    // Inventory return workflow configuration
    inventoryReturnBatchSize: 1, // process one inventory return at a time
    inventoryReturnMaxBatchingWindow: 5, // seconds
    incompleteBookingCleanupCadenceMinutes: 1, // how often to run the incomplete booking scraper and cleanup Lambda
    sparseGsi1Name: 'sparse-index-1',
    smsReminderBatchSize: 1, // process one SMS reminder at a time
    smsReminderMaxBatchingWindow: 5, // seconds
  },
  secrets: {
    merchantId: {
      name: 'merchantId',
    },
    hashKey: {
      name: 'hashKey',
    },
  },
};

class BookingWorkflowStack extends BaseStack {
  constructor(scope, primer) {
    super(scope, primer, defaults);

    logger.info(`Creating Booking Workflow Stack: ${this.stackId}`);

    // Resolve dependencies from other stacks
    const kmsKey = this.getConfigValue('enableTopicEncryption') ? scope.resolveKmsKey(this) : undefined;
    const transactionalDataTableName = scope.resolveTransDataTableName(this);
    const transDataTableArn = scope.resolveTransDataTableArn(this);
    const refDataTableName = scope.resolveRefDataTableName(this);
    const refDataTableArn = scope.resolveRefDataTableArn(this);

    // Resolve layers
    const baseLayer = scope.resolveBaseLayer(this);
    const awsUtilsLayer = scope.resolveAwsUtilsLayer(this);

    // Create environment-specific resource names
    const refundRequestTopicName = `${this.getAppName()}-${this.getDeploymentName()}-${this.getConfigValue('refundRequestTopicBaseName')}`;

    // Create SNS Topics

    // Refund Request Topic - Fan-out pattern for coordinated refunds
    this.refundRequestTopic = new Topic(this, this.getConstructId('refundRequestTopic'), {
      topicName: refundRequestTopicName,
      displayName: `${this.getAppName()} - ${this.getDeploymentName()} - Refund Requests`,
      masterKey: kmsKey,
    });

    // Create Dead Letter Queue for failed Worldline requests
    this.worldlineDeadLetterQueue = new Queue(this, this.getConstructId('worldlineDeadLetterQueue'), {
      // queueName: `${refundRequestTopicName}-worldline-dlq`, // Commented out to allow logical ID change
      retentionPeriod: Duration.days(14),
      // Add CloudWatch alarm for DLQ messages
      messageRetentionPeriod: Duration.days(14),
    });

    // Create SQS Queue for Worldline API requests
    this.worldlineRequestQueue = new Queue(this, this.getConstructId('worldlineRequestQueue'), {
      // queueName: `${refundRequestTopicName}-worldline-requests`, // Commented out to allow logical ID change
      visibilityTimeout: Duration.seconds(90),
      receiveMessageWaitTimeSeconds: 20,
      retentionPeriod: Duration.days(4),
      deadLetterQueue: {
        queue: this.worldlineDeadLetterQueue,
        maxReceiveCount: 3,
      },
    });

    // Create Dead Letter Queue for failed inventory return requests
    this.inventoryReturnDeadLetterQueue = new Queue(this, this.getConstructId('inventoryReturnDeadLetterQueue'), {
      retentionPeriod: Duration.days(14),
      messageRetentionPeriod: Duration.days(14),
    });

    this.inventoryReturnDeadLetterQueue.metricApproximateNumberOfMessagesVisible({ period: Duration.minutes(5) })
      .createAlarm(this, this.getConstructId('inventoryReturnDeadLetterAlarm'), {
        alarmName: `${this.getAppName()}-${this.getDeploymentName()}-inventory-return-dlq`,
        alarmDescription: 'An inventory return failed every retry and is waiting in the dead-letter queue',
        threshold: 1,
        evaluationPeriods: 1,
        comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
        treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
      });

    // Create SQS Queue for inventory return processing
    this.inventoryReturnQueue = new Queue(this, this.getConstructId('inventoryReturnQueue'), {
      visibilityTimeout: Duration.seconds(90),
      receiveMessageWaitTimeSeconds: 20,
      retentionPeriod: Duration.days(4),
      deadLetterQueue: {
        queue: this.inventoryReturnDeadLetterQueue,
        maxReceiveCount: 3,
      },
    });

    // Create Dead Letter Queue for failed SMS reminders
    this.smsReminderDeadLetterQueue = new Queue(this, this.getConstructId('smsReminderDeadLetterQueue'), {
      retentionPeriod: Duration.days(14),
    });

    // Create SQS Queue for SMS reminder processing
    this.smsReminderQueue = new Queue(this, this.getConstructId('smsReminderQueue'), {
      visibilityTimeout: Duration.seconds(90),
      receiveMessageWaitTimeSeconds: 20,
      retentionPeriod: Duration.days(4),
      deadLetterQueue: {
        queue: this.smsReminderDeadLetterQueue,
        maxReceiveCount: 3,
      },
    });

    // Create Lambda subscribers

    // Refund Subscriber Lambda
    this.refundSubscriber = new NodejsFunction(this, this.getConstructId('refundSubscriber'), {
      // functionName: `${this.getAppName()}-${this.getDeploymentName()}-RefundSubscriber`, // Commented out to allow logical ID change
      entry: path.join(__dirname, '../handlers/refundSubscriber/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          '@aws-sdk/*',
          '/opt/*'
        ],
        minify: false,
      },
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
        WORLDLINE_REQUEST_QUEUE_URL: this.worldlineRequestQueue.queueUrl,
      },
      layers: [baseLayer, awsUtilsLayer],
      timeout: Duration.seconds(60),
    });

    // Worldline Processor Lambda
    this.worldlineProcessor = new NodejsFunction(this, this.getConstructId('worldlineProcessor'), {
      // functionName: `${this.getAppName()}-${this.getDeploymentName()}-WorldlineProcessor`, // Commented out to allow logical ID change
      entry: path.join(__dirname, '../handlers/worldlineProcessor/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          '@aws-sdk/*',
          '/opt/*'
        ],
        minify: false,
      },
      timeout: Duration.seconds(60),
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
        MERCHANT_ID: this.getSecretValue('merchantId'),
        HASH_KEY: this.getSecretValue('hashKey'),
      },
      layers: [baseLayer, awsUtilsLayer],
      reservedConcurrentExecutions: 5,
    });

    // Connect SQS queue to Worldline processor Lambda
    this.worldlineProcessor.addEventSource(new SqsEventSource(this.worldlineRequestQueue, {
      batchSize: this.getConfigValue('worldlineDispatchBatchSize'),
      maxBatchingWindow: Duration.seconds(this.getConfigValue('worldlineDispatchMaxBatchingWindow')),
    }));

    // Inventory Return Processor Lambda
    this.inventoryReturnProcessor = new NodejsFunction(this, this.getConstructId('inventoryReturnProcessor'), {
      entry: path.join(__dirname, '../handlers/inventoryReturnProcessor/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          '@aws-sdk/*',
          '/opt/*'
        ],
        minify: false,
      },
      timeout: Duration.seconds(60),
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
        REFERENCE_DATA_TABLE_NAME: refDataTableName,
      },
      layers: [baseLayer, awsUtilsLayer],
    });

    this.smsReminderProcessor = new NodejsFunction(this, this.getConstructId('smsReminderProcessor'), {
      entry: path.join(__dirname, '../handlers/smsReminderProcessor/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          '/opt/*'
        ],
        minify: false,
      },
      timeout: Duration.seconds(60),
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        SMS_MESSAGE_TYPE: 'Transactional',
        ...(process.env.SMS_ORIGINATION_IDENTITY ? { SMS_ORIGINATION_IDENTITY: process.env.SMS_ORIGINATION_IDENTITY } : {}),
        ...(process.env.SMS_CONFIGURATION_SET_NAME ? { SMS_CONFIGURATION_SET_NAME: process.env.SMS_CONFIGURATION_SET_NAME } : {}),
        ...(process.env.SMS_PROTECT_CONFIGURATION_ID ? { SMS_PROTECT_CONFIGURATION_ID: process.env.SMS_PROTECT_CONFIGURATION_ID } : {}),
        ...(process.env.SMS_KEYWORD ? { SMS_KEYWORD: process.env.SMS_KEYWORD } : {}),
        ...(process.env.SMS_MAX_PRICE ? { SMS_MAX_PRICE: process.env.SMS_MAX_PRICE } : {}),
      },
      layers: [baseLayer],
    });

    // Connect SQS queue to Inventory Return Processor Lambda
    this.inventoryReturnProcessor.addEventSource(new SqsEventSource(this.inventoryReturnQueue, {
      batchSize: this.getConfigValue('inventoryReturnBatchSize'),
      maxBatchingWindow: Duration.seconds(this.getConfigValue('inventoryReturnMaxBatchingWindow')),
      reportBatchItemFailures: true
    }));

    this.smsReminderProcessor.addEventSource(new SqsEventSource(this.smsReminderQueue, {
      batchSize: this.getConfigValue('smsReminderBatchSize'),
      maxBatchingWindow: Duration.seconds(this.getConfigValue('smsReminderMaxBatchingWindow')),
    }));

    // Grant permissions to subscribers

    // DynamoDB permissions for refund subscriber
    const refundDynamoPolicy = new iam.PolicyStatement({
      actions: [
        'dynamodb:GetItem',
        'dynamodb:PutItem',
        'dynamodb:UpdateItem',
        'dynamodb:Query',
        'dynamodb:BatchWriteItem',
      ],
      resources: [transDataTableArn, `${transDataTableArn}/index/*`],
    });
    this.refundSubscriber.addToRolePolicy(refundDynamoPolicy);

    // SQS send permission for refund subscriber to queue Worldline requests
    const sqsSendPolicy = new iam.PolicyStatement({
      actions: ['sqs:SendMessage', 'sqs:GetQueueUrl'],
      resources: [this.worldlineRequestQueue.queueArn],
    });
    this.refundSubscriber.addToRolePolicy(sqsSendPolicy);

    // DynamoDB permissions for Worldline processor
    const worldlineDynamoPolicy = new iam.PolicyStatement({
      actions: [
        'dynamodb:GetItem',
        'dynamodb:UpdateItem',
        'dynamodb:Query',
      ],
      resources: [transDataTableArn, `${transDataTableArn}/index/*`],
    });
    this.worldlineProcessor.addToRolePolicy(worldlineDynamoPolicy);

    // DynamoDB permissions for inventory return processor
    const inventoryReturnDynamoPolicy = new iam.PolicyStatement({
      actions: [
        'dynamodb:GetItem',
        'dynamodb:PutItem',
        'dynamodb:UpdateItem',
        'dynamodb:Query',
        'dynamodb:BatchWriteItem',
        'dynamodb:TransactWriteItems',
      ],
      resources: [
        transDataTableArn,
        `${transDataTableArn}/index/*`,
        refDataTableArn,
        `${refDataTableArn}/index/*`,
      ],
    });
    this.inventoryReturnProcessor.addToRolePolicy(inventoryReturnDynamoPolicy);

    const smsSendPolicy = new iam.PolicyStatement({
      actions: ['sms-voice:SendTextMessage'],
      resources: ['*'],
    });
    this.smsReminderProcessor.addToRolePolicy(smsSendPolicy);

    // Subscribe Lambda functions to SNS topics
    // (Booking cancellation subscriber removed — cancellations are handled
    // synchronously in the cancel POST + expired-booking scraper now. Re-add
    // a subscriber here when the refund pipeline lands.)

    // Create DLQ for refund subscriber failures
    const refundSubscriberDLQ = new Queue(this, this.getConstructId('refundSubscriberDLQ'), {
      // queueName: `${refundRequestTopicName}-refund-subscriber-dlq`, // Commented out to allow logical ID change
      retentionPeriod: Duration.days(14),
    });

    // For refund subscriber queue (SNS -> SQS -> Lambda)
    const refundRequestQueue = new Queue(this, this.getConstructId('refundRequestQueue'), {
      // queueName: `${refundRequestTopicName}-refund-request-queue`, // Commented out to allow logical ID change
      visibilityTimeout: Duration.seconds(120), // 2 minutes for DB + SQS operations
      receiveMessageWaitTime: Duration.seconds(20),
      deadLetterQueue: {
        queue: refundSubscriberDLQ,
        maxReceiveCount: 3,
      },
    });

    this.refundRequestTopic.addSubscription(
      new SqsSubscription(refundRequestQueue, {
        rawMessageDelivery: false,
      })
    );

    // Connect the refund subscriber Lambda to the SQS queue
    this.refundSubscriber.addEventSource(new SqsEventSource(refundRequestQueue, {
      batchSize: this.getConfigValue('refundDispatchBatchSize'),
      maxBatchingWindow: Duration.seconds(this.getConfigValue('refundDispatchMaxBatchingWindow')),
    }));

    // Expired & Incomplete Booking Scraper schedule
    this.expiredBookingCleanupFn = new NodejsFunction(this, this.getConstructId('expiredBookingCleanupFn'), {
      entry: path.join(__dirname, '../handlers/incompleteBookingScraper/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: {
        externalModules: [
          '@aws-sdk/*',
          '/opt/*'
        ],
        minify: false,
      },
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
        REFERENCE_DATA_TABLE_NAME: refDataTableName,
        SPARSE_GSI1_NAME: this.getConfigValue('sparseGsi1Name') || 'sparse-index-1',
        INVENTORY_RETURN_QUEUE_URL: this.inventoryReturnQueue.queueUrl,
      },
      layers: [baseLayer, awsUtilsLayer],
      timeout: Duration.minutes(15),
      memorySize: 1024,
    });

    this.inventoryReturnQueue.grantSendMessages(this.expiredBookingCleanupFn);

    // Grant DynamoDB permissions to expired booking cleanup Lambda.
    // DeleteItem: the scraper removes the bookinghold:: marker of TIMED_OUT bookings.
    this.expiredBookingCleanupFn.addToRolePolicy(new iam.PolicyStatement({
      actions: [
        'dynamodb:GetItem',
        'dynamodb:PutItem',
        'dynamodb:UpdateItem',
        'dynamodb:DeleteItem',
        'dynamodb:Query',
        'dynamodb:BatchWriteItem',
        'dynamodb:TransactWriteItems',
      ],
      resources: [transDataTableArn, `${transDataTableArn}/index/*`],
    }));

    this.expiredBookingCleanupFn.addToRolePolicy(new iam.PolicyStatement({
      actions: [
        'dynamodb:GetItem',
        'dynamodb:PutItem',
        'dynamodb:UpdateItem',
        'dynamodb:Query',
        'dynamodb:BatchWriteItem',
        'dynamodb:TransactWriteItems',
      ],
      resources: [refDataTableArn, `${refDataTableArn}/index/*`],
    }));

    // Live inventory as metrics, every five minutes: availability and
    // capacity per activity and product for today and the next week, written
    // with PutMetricData because they carry dimensions (see the handler).
    const metricNamespace = eventMetricNamespace(this.getDeploymentName());
    this.inventoryMetricsFn = new NodejsFunction(this, this.getConstructId('inventoryMetricsFn'), {
      entry: path.join(__dirname, '../handlers/inventoryMetrics/index.js'),
      handler: 'handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      bundling: { externalModules: ['@aws-sdk/*', '/opt/*'], minify: false },
      environment: {
        LOG_LEVEL: this.getConfigValue('logLevel'),
        REFERENCE_DATA_TABLE_NAME: refDataTableName,
        METRIC_NAMESPACE: metricNamespace,
        HORIZON_DAYS: String(this.getConfigValue('inventoryMetricsHorizonDays') || 7),
      },
      layers: [baseLayer, awsUtilsLayer],
      timeout: Duration.minutes(2),
      memorySize: 512,
    });
    this.inventoryMetricsFn.addToRolePolicy(new iam.PolicyStatement({
      actions: ['dynamodb:Scan'],
      resources: [refDataTableArn],
    }));
    this.inventoryMetricsFn.addToRolePolicy(new iam.PolicyStatement({
      actions: ['cloudwatch:PutMetricData'],
      resources: ['*'],
      conditions: { StringEquals: { 'cloudwatch:namespace': metricNamespace } },
    }));
    new events.Rule(this, this.getConstructId('inventoryMetricsSchedule'), {
      schedule: events.Schedule.rate(Duration.minutes(5)),
      targets: [new targets.LambdaFunction(this.inventoryMetricsFn)],
    });

    // Add EventBridge Rule to trigger incomplete booking scraper and cleanup Lambda
    this.cleanupRule = new events.Rule(this, this.getConstructId('incompleteBookingScraperSchedule'), {
      schedule: events.Schedule.rate(this.getConfigValue('incompleteBookingCleanupCadenceMinutes') ? Duration.minutes(this.getConfigValue('incompleteBookingCleanupCadenceMinutes')) : Duration.minutes(15)),
      description: 'Triggers the expired Bookings scraper to reclaim expired booking sessions and release held inventory.',
      enabled: true,
    });

    // Attach the cleanup Lambda as the target of the EventBridge rule with retry attempts
    this.cleanupRule.addTarget(new targets.LambdaFunction(this.expiredBookingCleanupFn, {
      retryAttempts: 2,
    }));

    // Export References for other stacks to consume

    // Refund Request Topic ARN
    this.exportReference(this, 'refundRequestTopicArn', this.refundRequestTopic.topicArn,
      `ARN of the refund request topic in ${this.stackId}`);

    // SMS Reminder Queue URL
    this.exportReference(this, 'smsReminderQueueUrl', this.smsReminderQueue.queueUrl,
      `URL of the SMS reminder queue in ${this.stackId}`);

    // SMS Reminder Queue ARN
    this.exportReference(this, 'smsReminderQueueArn', this.smsReminderQueue.queueArn,
      `ARN of the SMS reminder queue in ${this.stackId}`);

    // Bind resolver functions to scope for other stacks to consume
    scope.resolveRefundRequestTopic = this.resolveRefundRequestTopic.bind(this);
    scope.resolveRefundRequestTopicArn = this.resolveRefundRequestTopicArn.bind(this);
    scope.resolveSmsReminderQueueUrl = this.resolveSmsReminderQueueUrl.bind(this);
    scope.resolveSmsReminderQueueArn = this.resolveSmsReminderQueueArn.bind(this);

    // Direct getters (for constructs within the same deployment)
    scope.getRefundRequestTopic = this.getRefundRequestTopic.bind(this);
    scope.getSmsReminderQueue = this.getSmsReminderQueue.bind(this);
  }

  // Getters for resources (used within the same deployment)
  getRefundRequestTopic() {
    return this.refundRequestTopic;
  }

  getSmsReminderQueue() {
    return this.smsReminderQueue;
  }

  // Resolver functions for cross-stack references
  resolveRefundRequestTopic(consumerScope) {
    const topicArn = this.resolveReference(consumerScope, this.getExportSSMPath('refundRequestTopicArn'));
    return Topic.fromTopicArn(consumerScope, `${this.getConstructId('refundRequestTopic')}-imported`, topicArn);
  }

  resolveRefundRequestTopicArn(consumerScope) {
    return this.resolveReference(consumerScope, this.getExportSSMPath('refundRequestTopicArn'));
  }

  resolveSmsReminderQueueUrl(consumerScope) {
    return this.resolveReference(consumerScope, this.getExportSSMPath('smsReminderQueueUrl'));
  }

  resolveSmsReminderQueueArn(consumerScope) {
    return this.resolveReference(consumerScope, this.getExportSSMPath('smsReminderQueueArn'));
  }
}

async function createBookingWorkflowStack(scope, stackKey) {
  try {
    const primer = new StackPrimer(scope, stackKey, defaults);
    await primer.prime();
    return new BookingWorkflowStack(scope, primer);
  } catch (error) {
    throw new Error(`Error creating Booking Workflow Stack: ${error}`);
  }
}

module.exports = {
  createBookingWorkflowStack
};
