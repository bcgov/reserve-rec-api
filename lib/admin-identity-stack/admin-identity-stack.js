...
class AdminIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    const bceidMetadataEnv = ['dev', 'test', 'prod'].includes(this.getDeploymentName()) ? this.getDeploymentName() : 'dev';

    const bceidMetadataPath = path.join(__dirname, './config/', `bceid-${bceidMetadataEnv}.xml`)

    if (!fs.existsSync(bceidMetadataPath)) {
      throw new Error(`BCeID metadata file not found for deployment ${bceidMetadataEnv}: ${bceidMetadataPath}`);
    }
    ...
    this.bceidSamlProvider = new cognito.UserPoolIdentityProviderSaml(this, this.getConstructId('bceidSamlProvider'), {
      ...
      attributeMapping: {
        email: cognito.ProviderAttribute.other('email'),
        fullname: cognito.ProviderAttribute.other('displayname'),
      }
    });
