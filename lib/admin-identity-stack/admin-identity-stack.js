...
const { createConstruct, exportValue, getContextFromSSM, getSyncSecretFromSM, logger, stackNamer } = require('../utils');
  ...
  constructor(scope, id, props) {
    ...
    const userPoolDomain = createConstruct(this, userPoolDomainName, {
      createFn: (id) => new cognito.UserPoolDomain(this, id, {
        userPool: adminUserPool,
        cognitoDomain: {
          domainPrefix: `${stackConstants?.COGNITO_DOMAIN_PREFIX}-${props?.ENVIRONMENT_NAME}`
        }
      }),
      exportFn: (construct) => exportValue(this, userPoolDomainName, {
        value: construct.baseUrl(),
        pathSuffix: 'domain',
        ...props
      }),
      ...props
