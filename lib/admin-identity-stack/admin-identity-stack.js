...
class AdminIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    const bceidMetadataPath = path.join(__dirname, './config/', `bceid-${this.getDeploymentName()}.xml`)

    if (!fs.existsSync(bceidMetadataPath)) {
      throw new Error(`BCeID metadata file not found for deployment ${this.getDeploymentName()}: ${bceidMetadataPath}`);
    }
    
    const bceidMetadataXml = fs.readFileSync(bceidMetadataPath, 'utf8');
    ...
    if (this.overrides?.skipCreation === true) {
      ...
      // Import dev IAM roles into CDK constructs so downstream stacks (e.g. openSearchStack)
      // can call .addManagedPolicy() and .addToPolicy() on them
      if (this.overrides.superAdminRoleArn) {
        scope.addRole('superAdmin', iam.Role.fromRoleArn(this, 'ImportedSuperAdminRole', this.overrides.superAdminRoleArn));
      }
      if (this.overrides.administratorRoleArn) {
        scope.addRole('administrator', iam.Role.fromRoleArn(this, 'ImportedAdministratorRole', this.overrides.administratorRoleArn));
      }
      if (this.overrides.parkOperatorRoleArn) {
    ...
    this.adminUserPool = new cognito.UserPool(this, this.getConstructId('adminUserPool'), {
      ...
      deletionProtection: this.isSandbox() ? false : (this.getConfigValue('poolDeletionProtection') === 'true' ? true : false),
    ...
    // Check if the BCeID flag in config is set - if it is, import the provider instead of creating a new one
    const useExistingBceid = this.getConfigValue('useExistingBceid') || this.overrides?.useExistingBceid;
    if (useExistingBceid) {
      this.bceidSamlProvider = cognito.UserPoolIdentityProvider.fromProviderName(
        this,
        this.getConstructId('bceidSamlProvider'),
        'BCeID'
      );
    } else {
      this.bceidSamlProvider = new cognito.UserPoolIdentityProviderSaml(this, this.getConstructId('bceidSamlProvider'), {
        userPool: this.adminUserPool,
        name: 'BCeID',
        metadata: cognito.UserPoolIdentityProviderSamlMetadata.file(bceidMetadataXml),
        idpInitiated: true,
        attributeMapping: {
          email: cognito.ProviderAttribute.other('email'),
          name: cognito.ProviderAttribute.other('displayname'),
          'custom:GUID': cognito.ProviderAttribute.other('GUID'),
        }
      });
    }
    ...
    this.adminUserPoolClient = new cognito.UserPoolClient(this, this.getConstructId('adminUserPoolClient'), {
      ...
      supportedIdentityProviders: [
        ...
        cognito.UserPoolClientIdentityProvider.custom(this.bceidSamlProvider.providerName),
      ],
      oAuth: {
        flows: {
      ...
      customAttributes: {
        ...
        GUID: new cognito.StringAttribute({ mutable: true }),
      },
    ...
    this.adminUserPoolClient.node.addDependency(this.bceidSamlProvider);
