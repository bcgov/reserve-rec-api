...
const { stubFunction } = require('./utils'); // Import the stub function
...
class ReserveRecCdkStack extends Stack {
  constructor(scope, id, props) {
    ...
    const streamRole = new iam.Role(this, 'StreamRole', {
      ...
      roleName: 'ReserveRecStreamRole',
      description: 'Role for Lambda to access DynamoDB Streams and OpenSearch',
    });

    streamRole.addToPolicy(streamRoleDynamoDBPolicy);
    streamRole.addToPolicy(streamRoleOSPolicy);
    streamRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaBasicExecutionRole'));
    streamRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaDynamoDBExecutionRole'));
    streamRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName('service-role/AWSLambdaVPCAccessExecutionRole'));
    ...
    const opensearchDomain = new opensearch.Domain(this, 'OpenSearchDomain', {
      ...
      fineGrainedAccessControl: {
        masterUserArn: streamRole.roleArn,
      },
      zoneAwareness: {
        enabled: false,
      },
    ...
    if (props.env.IS_OFFLINE !== 'true') {
      props.env.OPENSEARCH_DOMAIN_URL = `https://${opensearchDomain.domainEndpoint}`;
    }

    // Tooling roles

    const osMasterPolicy = new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ['es:*'],
      resources: [opensearchDomain.domainArn],
    });

    const osMasterRole = new iam.Role(this, 'OSMasterRole', {
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
      roleName: 'ReserveRecOSMasterRole',
      description: 'Role for Lambda to create OpenSearch index mappings',
    });

    osMasterRole.addToPolicy(osMasterPolicy);
    ...
    // IMPORTED RESOURCES (ALPHABETIZED)
