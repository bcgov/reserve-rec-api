...
const defaults = {
  ...
  constructs: {
    ...
    publicTransDataReadRole: {
      name: 'PublicTransDataReadRole',
    }
  },
  config: {
...
class TransactionalDataStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Add sparse GSI for bookings
    this.transDataTable.addGlobalSecondaryIndex({
      indexName: this.getConstructName('sparseGsi1'),
      partitionKey: { name: 'isPending', type: dynamodb.AttributeType.STRING },
      sortKey: { name: 'sessionExpiry', type: dynamodb.AttributeType.NUMBER },
    ...
    this.dynamoStreamConstruct = new DynamoStreamConstruct(this, this.getConstructId('dynamoStream'), {
      ...
      environment: {
        ...
        OPENSEARCH_USER_INDEX_NAME: scope.resolveOpenSearchUserIndexName(this),
      },
      layers: [
        baseLayer,
        awsUtilsLayer
