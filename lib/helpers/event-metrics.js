...
const EVENT_GROUPS = {
  ...
  signup: ['signup_refused', 'account_confirmed', 'account_created'],
  emailChange: [
    'email_changed',
...
//   unconfirmed_users                pool users who signed up and never confirmed
//   estimated_users                  the pool's own user count estimate
const GAUGES = ['unconfirmed_users', 'estimated_users'];
...
/** One dashboard per environment: a graph per group, and one for the gauges. */
function addEventDashboard(scope, id, namespace) {
  ...
  const gauges = new cloudwatch.GraphWidget({
    title: 'Accounts',
    left: GAUGES.map((name) => new cloudwatch.Metric({
      namespace,
      metricName: name,
      statistic: 'Maximum',
