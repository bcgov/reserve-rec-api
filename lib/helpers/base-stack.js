...
class BaseStack extends Stack {
  constructor(appScope, primer, props = {}) {
    ...
    const envName = appScope.getDeploymentName();

    super(appScope.app, primer?.stackId, {
      ...props,
      env: primer?.env,
      description: primer?.description + ` (${envName})` || `Stack for ${primer?.stackKey || 'DEFAULT_STACK'}`,
    });
    ...
    this.getSandboxName = appScope.getSandboxName.bind(appScope);
    this.isSandbox = appScope.isSandbox.bind(appScope);
  ...
  buildStackTriggerTagger(){
    this.noOpProvider = new Provider(this, this.stackId + '-NoOpProviderV2', {
      onEventHandler: new lambda.Function(this, this.stackId + '-NoOpFunctionV2', {
  ...
  addTriggerTag(scope, tagKey, tagValue) {
    ...
    new CustomResource(scope, `${uniqueTagKey}-NoOpResourceV2`, {
      serviceToken: this.noOpProvider.serviceToken,
      properties: {
