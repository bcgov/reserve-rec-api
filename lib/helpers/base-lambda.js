...
const DEFAULT_LAMBDA_MEMORYSIZE= 128;
...
class LambdaConstruct extends Construct {
  constructor(scope, id, props) {
    ...
    this.id = id;
    // Use concreteStackId if available (for NestedStacks), otherwise stackId
    this.stackId = scope.concreteStackId || scope.stackId;
  ...
  createDefaultEnvironmentVariables() {
    return {
      ...
      OPENSEARCH_DOMAIN_ENDPOINT: `https://${this.appScope.resolveOpenSearchDomainEndpoint(this)}`,
      OPENSEARCH_REFERENCE_DATA_INDEX_NAME: this.appScope.resolveOpenSearchReferenceDataIndexName(this),
      OPENSEARCH_TRANSACTIONAL_DATA_INDEX_NAME: this.appScope.resolveOpenSearchTransactionalDataIndexName(this),
    };
  }
  ...
  // A helper method to add CORS preflight OPTIONS method to a list of API Gateway resources
  addCorsPreflightForResources(resources = []) {
    ...
    for (const resource of resources) {
      ...
      const path = resource.path || resource.node.path;
  ...
  // Get Audit Table ARN
  getAuditTableArn() {
    if (this.registry.has('AuditTableArn')) {
      return this.registry.get('AuditTableArn');
    }
    const auditTableArn = this.appScope.resolveAuditTableArn(this);
    this.registry.set('AuditTableArn', auditTableArn);
    return auditTableArn;
  }

  // Grant write access to the audit table
  grantAuditTableWrite(lambdaFunction) {
    const auditTableArn = this.getAuditTableArn();
    const writePolicy = new iam.PolicyStatement({
      actions: [
        'dynamodb:PutItem',
        'dynamodb:BatchWriteItem',
      ],
      resources: [auditTableArn, `${auditTableArn}/index/*`],
    });
    ...
    try {
      const fn = new NodejsFunction(scope, functionId, {
        ...
        bundling: {
          ...
          logLevel: LogLevel.ERROR, // Changed from SILENT to show bundling errors
        },
        layers: this.resolveLayers(),
        environment: this.resolveEnvironment(),
        description: props?.description || `Lambda function ${functionId} for Admin API`,
        functionName: functionId,
        memorySize: props?.memorySize || DEFAULT_LAMBDA_MEMORYSIZE,
