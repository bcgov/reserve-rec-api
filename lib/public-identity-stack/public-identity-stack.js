...
class PublicIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // src/layers/base/emailBlocklist.js). The SSM parameter stays a second
    // source until every environment's table is seeded; then the SSM read
    // goes. The table holds defence data: retained, never in the repo.
    ...
    const blocklistParam = this.getConfigValue('blocklistSsmParam') || ssmPath('signup', 'emailBlocklist');
    const blocklistEnv = {
      BLOCKLIST_TABLE_NAME: this.emailBlocklistTable.tableName,
      BLOCKLIST_SSM_PARAM: blocklistParam,
    };
    const blocklistRead = (fn) => {
