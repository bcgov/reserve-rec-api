...
class PublicIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // // Create general public user pool group
    // this.publicUserGroup = new cognito.CfnUserPoolGroup(this, this.getConstructId('publicUserGroup'), {
    //   groupName: this.getConstructId('publicUserGroup'),
    //   userPoolId: this.publicUserPool.userPoolId,
    //   description: 'General public users group',
    // });
    ...
    // Bind exports for other stacks to consume
    scope.getPublicUserPoolId = this.getPublicUserPoolId.bind(this);
    scope.getPublicUserPool = this.getPublicUserPool.bind(this);
    scope.getPublicUserPoolClientId = this.getPublicUserPoolClientId.bind(this);
    scope.getPublicCognitoDomain = this.getPublicCognitoDomain.bind(this);
    scope.getPublicIdentityPoolId = this.getPublicIdentityPoolId.bind(this);
    scope.getPublicUserGroup = this.getPublicUserGroup.bind(this);

    // Cfn Outputs
    new CfnOutput(this, `${this.getConstructId('publicUserPool')}-cfnOutput`, {
      value: this.publicUserPool.userPoolId,
      exportName: `${this.getCfnPath('publicUserPool')}-userPoolId`
    });

    new CfnOutput(this, `${this.getConstructId('publicUserPoolClient')}-cfnOutput`, {
      value: this.publicUserPoolClient.userPoolClientId,
      exportName: `${this.getCfnPath('publicUserPoolClient')}-userPoolClientId`
    });

    new CfnOutput(this, `${this.getConstructId('publicUserPoolDomain')}-cfnOutput`, {
      value: this.publicUserPoolDomain.domainName,
      exportName: `${this.getCfnPath('publicUserPoolDomain')}-domainName`
    });

    new CfnOutput(this, `${this.getConstructId('publicIdentityPool')}-cfnOutput`, {
      value: this.publicIdentityPool.ref,
      exportName: `${this.getCfnPath('publicIdentityPool')}-identityPoolId`
    });
