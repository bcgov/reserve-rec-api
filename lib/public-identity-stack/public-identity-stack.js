...
class PublicIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Public User Pool ID
    // The BCSC email_verified repair writes back to this pool. Granted after
    // the pool exists; the policy is its own resource, so there is no cycle
    // with the pool referencing the function.
    this.preTokenGenerationFunction.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
