...
class PublicIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Create POST_CONFIRMATION Lambda trigger to write new users to DynamoDB
    ...
    this.newUserRegisterFunction = new lambda.Function(this, this.getConstructId('newUserRegisterFunction'), {
      ...
      description: `New user registration trigger - writes user data to DynamoDB when users confirm their account`,
    ...
    // Grant Cognito permissions (needed to return the event)
    this.newUserRegisterFunction.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: [
    ...
    // Grant the User Pool permission to invoke the Lambda
    this.newUserRegisterFunction.grantInvoke(
    ...
    logger.info('POST_CONFIRMATION trigger configured - new users will be written to DynamoDB');
