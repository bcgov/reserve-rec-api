...
const { addEventMetrics, eventMetricNamespace } = require('../helpers/event-metrics');
const { addAccountsDashboard } = require('../helpers/dashboards');
...
const defaults = {
  ...
  constructs: {
    ...
    preSignUpPoolPolicy: {
      name: 'PreSignUpPoolPolicy',
    },
    preTokenGenerationFunction: {
      name: 'PreTokenGeneration',
    },
    preTokenGenerationPoolPolicy: {
      name: 'PreTokenGenerationPoolPolicy',
    },
    customMessageFunction: {
      name: 'CustomMessage',
    },
    cognitoAuditFunction: {
      name: 'CognitoAudit',
    },
    cognitoAuditRule: {
      name: 'CognitoAuditRule',
    },
    accountConfirmedFunction: {
      name: 'AccountConfirmed',
    },
    userStatusCountFunction: {
      name: 'UserStatusCount',
    },
    userStatusCountRule: {
      name: 'UserStatusCountRule',
    },
    eventDashboard: {
      name: 'EventDashboard',
    },
  ...
  config: {
    ...
    // Email-change refusal ships disabled: detection logs first, the switch
    // is thrown once the soak shows it bites only what it should.
    emailChangeRefuse: 'false',
    emailChangeRefuseBcsc: 'false',
    // One account per mailbox. Off, a duplicate is logged and allowed.
    duplicateEmailRefuse: 'true',
    emailChangeVeto: 'false',
    cognitoAuditEnabled: 'true',
...
class PublicIdentityStack extends BaseStack {
  constructor(scope, primer) {
    ...
    if (this.overrides?.skipCreation === true) {
      ...
      // Import dev IAM role into CDK construct so downstream stacks (e.g. openSearchStack)
      // can call .addManagedPolicy() and .addToPolicy() on it
      if (this.overrides.publicRoleArn) {
        scope.addRole('public', iam.Role.fromRoleArn(this, 'ImportedPublicRole', this.overrides.publicRoleArn));
      }
    ...
    // Branded verification email (#460). emailAssetBaseUrl points at the public
    // app's hosted copies of the template's images — e.g.
    // https://test-reserve.bcparks.ca/dayuse/assets/email. Unset leaves the
    // pool on Cognito's default email (bcgov/reserve-rec-public#502).
    const verificationEmailBody = buildVerificationEmail(this.getConfigValue('emailAssetBaseUrl'));

    // The triggers live in this stack because this stack owns the pool.
    // UpdateUserPool replaces the whole configuration, and CloudFormation sends
    // its own view of the resource on every property change — so a trigger
    // attached from any other stack is dropped the first time this pool's
    // template changes. That happened on dev. One writer, or none survive.
    //
    // The transactional table is read from SSM rather than referenced: a hard
    // reference would cycle (transactional → opensearch → this stack). The name
    // is deterministic and already exported there.
    const baseLayer = scope.resolveBaseLayer(this);
    const awsUtilsLayer = scope.resolveAwsUtilsLayer(this);
    const ssmPath = (...parts) => '/' + [this.getAppName(), this.getDeploymentName(), ...parts]
      .map((x) => x.charAt(0).toLowerCase() + x.slice(1)).join('/');
    const transDataTableName = this.resolveReference(this, ssmPath('transactionalDataStack', 'transactionalDataTableName'));
    const transDataTableArn = `arn:aws:dynamodb:${this.region}:${this.account}:table/${transDataTableName}`;
    // Email blocklist: one item per banned address, domain or pattern (see
    // src/layers/base/emailBlocklist.js). Holds defence data: retained, never
    // in the repo.
    this.emailBlocklistTable = new dynamodb.Table(this, this.getConstructId('emailBlocklistTable'), {
      tableName: this.getConstructId('emailBlocklistTable'),
    ...
    const blocklistEnv = { BLOCKLIST_TABLE_NAME: this.emailBlocklistTable.tableName };
    // One item per canonical mailbox, naming the address whose account holds
    // it (see the PreSignUp handler). User PII, so kept apart from the
    // blocklist's defence data.
    this.emailClaimTable = new dynamodb.Table(this, this.getConstructId('emailClaimTable'), {
      tableName: this.getConstructId('emailClaimTable'),
      partitionKey: { name: 'pk', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      pointInTimeRecoverySpecification: { pointInTimeRecoveryEnabled: true },
      removalPolicy: RemovalPolicy.RETAIN,
    });
    this.signupRefusalTable = new dynamodb.Table(this, this.getConstructId('signupRefusalTable'), {
      tableName: this.getConstructId('signupRefusalTable'),
      partitionKey: { name: 'pk', type: dynamodb.AttributeType.STRING },
      sortKey: { name: 'sk', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
    ...
    const blocklistRead = (fn) => fn.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ['dynamodb:Query'],
      resources: [this.emailBlocklistTable.tableArn],
    }));

    // Both triggers load the DynamoDB SDK from the layer; at the 128MB default
    // that left ~20MB of headroom and a Query took over four seconds. Cognito
    // waits at most five seconds for a trigger, so memory (and with it CPU) is
    // the lever, not the timeout.
    const TRIGGER_MEMORY_MB = 512;

    // PreSignUp — refuse account creation for blocked addresses and for a
    // second account on one mailbox.
    this.preSignUpFunction = new lambda.Function(this, this.getConstructId('preSignUpFunction'), {
      ...
      memorySize: TRIGGER_MEMORY_MB,
      layers: [baseLayer, awsUtilsLayer],
      logRetention: EVENT_LOG_RETENTION,
      environment: {
        ...blocklistEnv,
        CLAIM_TABLE_NAME: this.emailClaimTable.tableName,
        REFUSAL_TABLE_NAME: this.signupRefusalTable.tableName,
        DUPLICATE_EMAIL_REFUSE: this.getConfigValue('duplicateEmailRefuse'),
        LOG_LEVEL: this.getConfigValue('logLevel'),
      },
    });
    blocklistRead(this.preSignUpFunction);
    this.emailClaimTable.grant(this.preSignUpFunction, 'dynamodb:PutItem');
    this.signupRefusalTable.grant(this.preSignUpFunction, 'dynamodb:PutItem');

    // PreTokenGeneration — fires on every authentication. Creates the user
    // record on first login and restores email_verified for BCSC accounts,
    // which Cognito resets on every federated login. Directory name is
    // historical.
    this.preTokenGenerationFunction = new lambda.Function(this, this.getConstructId('preTokenGenerationFunction'), {
      functionName: this.getConstructId('preTokenGenerationFunction'),
      handler: 'index.handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      ...
      environment: {
        ...
        ...blocklistEnv,
        EMAIL_CHANGE_REFUSE: this.getConfigValue('emailChangeRefuse'),
        EMAIL_CHANGE_REFUSE_BCSC: this.getConfigValue('emailChangeRefuseBcsc'),
        LOG_LEVEL: this.getConfigValue('logLevel'),
      },
    });
    this.preTokenGenerationFunction.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ['dynamodb:GetItem', 'dynamodb:PutItem', 'dynamodb:UpdateItem'],
      resources: [transDataTableArn],
    }));
    blocklistRead(this.preTokenGenerationFunction);

    // CustomMessage — a flagged veto on self-service email changes. Returns
    // every event untouched, so the pool's own message templates stay in
    // force; when EMAIL_CHANGE_VETO is on it throws on UpdateUserAttribute,
    // which (observed, not documented — see the handler) stops the pending
    // value being stored. Needs nothing beyond the base layer.
    this.customMessageFunction = new lambda.Function(this, this.getConstructId('customMessageFunction'), {
      functionName: this.getConstructId('customMessageFunction'),
      handler: 'index.handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      code: lambda.Code.fromAsset('lib/handlers/cognitoTriggers/customMessage'),
      timeout: Duration.seconds(5),
      layers: [baseLayer],
      logRetention: EVENT_LOG_RETENTION,
      environment: {
        EMAIL_CHANGE_VETO: this.getConfigValue('emailChangeVeto'),
        LOG_LEVEL: this.getConfigValue('logLevel'),
      },
    });

    // PostConfirmation — one log line per confirmed signup. Nothing else in
    // the pool counts confirmations. Needs nothing beyond the base layer and
    // never throws.
    this.accountConfirmedFunction = new lambda.Function(this, this.getConstructId('accountConfirmedFunction'), {
      functionName: this.getConstructId('accountConfirmedFunction'),
      handler: 'index.handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      code: lambda.Code.fromAsset('lib/handlers/cognitoTriggers/accountConfirmed'),
      timeout: Duration.seconds(5),
      layers: [baseLayer],
      logRetention: EVENT_LOG_RETENTION,
      environment: {
    ...
    this.publicUserPool = new cognito.UserPool(this, this.getConstructId('publicUserPool'), {
      ...
      lambdaTriggers: {
        ...
        customMessage: this.customMessageFunction,
        postConfirmation: this.accountConfirmedFunction,
      },
      ...
      email: userPoolFromEmail
        ? cognito.UserPoolEmail.withSES({
            ...
            // Defaults to the sender, so a reply lands somewhere rather than
            // nowhere. Set userPoolReplyToEmail to route replies elsewhere.
            replyTo: this.getConfigValue('userPoolReplyToEmail') || userPoolFromEmail,
      ...
      userVerification: {
        ...
        // Use the branded template when the images have somewhere to be served
        // from; without emailAssetBaseUrl set, stay on Cognito's default rather
        // than send a branded email full of broken images
        // (bcgov/reserve-rec-public#502).
        ...(verificationEmailBody
          ? {
              emailSubject: 'Verify your e-mail with BC Parks',
      ...
      // Keep the old address active and verified until the new one is
      // confirmed from its own inbox. The prop this replaced,
      // userAttributeUpdateSettings, is not a CDK prop and applied nothing;
      // prod carried the setting by hand from 4 Sep. The template owns it now.
    ...
    this.bcscOIDCProvider = new cognito.UserPoolIdentityProviderOidc(this, this.getConstructId('bcscOIDCProvider'), {
      ...
      attributeMapping: {
        ...
        address: cognito.ProviderAttribute.other("address"),
      }
    });
    ...
    // Public User Pool ID
    // The BCSC email_verified repair writes back to this pool. This must NOT
    // go through addToRolePolicy: that merges into the role's DefaultPolicy,
    // which the Function is made to depend on — and the pool depends on the
    // Function through lambdaTriggers, so referencing the pool ARN from there
    // is a cycle. A standalone Policy attaches to the role without the
    // Function depending on it.
    new iam.Policy(this, this.getConstructId('preTokenGenerationPoolPolicy'), {
      roles: [this.preTokenGenerationFunction.role],
      statements: [new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: ['cognito-idp:AdminUpdateUserAttributes'],
        resources: [this.publicUserPool.userPoolArn],
      })],
    });

    // Same cycle as above: the claim check looks up the claim's holder.
    new iam.Policy(this, this.getConstructId('preSignUpPoolPolicy'), {
      roles: [this.preSignUpFunction.role],
      statements: [new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
    ...
    // CognitoAudit — logs every email attribute change on the pool from the
    // CloudTrail record, including admin and console edits the triggers never
    // see. Read-only: no SSM, no writes. Not a pool trigger, so referencing
    // the pool id from its environment does not cycle.
    this.cognitoAuditFunction = new lambda.Function(this, this.getConstructId('cognitoAuditFunction'), {
      functionName: this.getConstructId('cognitoAuditFunction'),
      handler: 'index.handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      code: lambda.Code.fromAsset('lib/handlers/cognitoTriggers/cognitoAudit'),
      timeout: Duration.seconds(10),
      layers: [baseLayer],
      logRetention: EVENT_LOG_RETENTION,
      environment: {
        PUBLIC_USER_POOL_ID: this.publicUserPool.userPoolId,
        // PreTokenGeneration forces email_verified for BCSC accounts on every
        // login; the audit skips admin calls whose session name is that function.
        SELF_ROLE_NAME_FRAGMENT: this.getConstructId('preTokenGenerationFunction'),
        LOG_LEVEL: this.getConfigValue('logLevel'),
      },
    });

    const metricNamespace = eventMetricNamespace(this.getDeploymentName());

    // UserStatusCount — hourly gauge of unconfirmed users and the pool's
    // estimated total, written with PutMetricData (see event-metrics.js for
    // why not a filter). Not a trigger, so it may reference the pool.
    this.userStatusCountFunction = new lambda.Function(this, this.getConstructId('userStatusCountFunction'), {
      functionName: this.getConstructId('userStatusCountFunction'),
      handler: 'index.handler',
      runtime: lambda.Runtime.NODEJS_20_X,
      code: lambda.Code.fromAsset('lib/handlers/cognitoTriggers/userStatusCount'),
    ...
    this.userStatusCountFunction.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
    ...
    new events.Rule(this, this.getConstructId('userStatusCountRule'), {
      ruleName: this.getConstructId('userStatusCountRule'),
      schedule: events.Schedule.rate(Duration.hours(1)),
      targets: [new targets.LambdaFunction(this.userStatusCountFunction)],
    });

    // Metric filters for the event= lines these triggers log, and the
    // per-environment dashboard that reads them (lib/helpers/event-metrics.js).
    const eventAlarms = this.getConfigValue('eventAlarms') || {};
    addEventMetrics(this.preSignUpFunction, metricNamespace, ['signup_refused', 'signup_phone_refused'], eventAlarms);
    addEventMetrics(this.accountConfirmedFunction, metricNamespace, ['account_confirmed'], eventAlarms);
    addEventMetrics(this.preTokenGenerationFunction, metricNamespace,
      ['account_created', 'email_changed', 'email_change_refused'], eventAlarms);
    addEventMetrics(this.customMessageFunction, metricNamespace,
      ['email_change_observed', 'email_change_vetoed'], eventAlarms);
    addEventMetrics(this.cognitoAuditFunction, metricNamespace, [
    ...
    addAccountsDashboard(this, this.getConstructId('accountsDashboard'), metricNamespace, {
      userPoolId: this.publicUserPool.userPoolId,
      userPoolClientId: this.publicUserPoolClient.userPoolClientId,
    });

    if (this.getConfigValue('cognitoAuditEnabled') !== 'false') {
      // Prerequisite: a CloudTrail trail recording management events in this
      // region must exist, or EventBridge receives nothing. That trail is
      // account-level and is not created here.
      this.cognitoAuditRule = new events.Rule(this, this.getConstructId('cognitoAuditRule'), {
        ruleName: this.getConstructId('cognitoAuditRule'),
        eventPattern: {
      ...
      this.cognitoAuditRule.addTarget(new targets.LambdaFunction(this.cognitoAuditFunction));
    }
