...
class PublicIdentityIntegrationStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // UpdateUserPool refuses that without iam:PassRole on the role. Scoped to
    // the service-role path Cognito's SMS roles live under. An
    // iam:PassedToService condition was tried first and denied — not every
    // service populates that key, and an absent key fails a StringEquals.
    this.attachTriggersFunction.addToRolePolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ['iam:PassRole'],
      resources: [`arn:aws:iam::${this.account}:role/service-role/*`]
    }));
