...
const { GEOZONES_API_PUT_CONFIG } = require("/opt/geozones/configs");
  ...
  try {
    ...
    for (const item of body) {
      ...
      if (!item?.identifier && !item?.geozoneId) {
        throw new Exception('Either identifier or geozoneID is required for every post item', { code: 400 });
      }
      ...
      item.pk = pk
      item.sk = sk
      item.gzCollectionId = item?.orcs ? item.orcs : `${gzCollection}::${gzCollectionId}`;
      item.orcs = item?.orcs ? item.orcs : Number(gzCollectionId);
    ...
    const putItems = await quickApiPutHandler(TABLE_NAME, putRequests, GEOZONES_API_PUT_CONFIG);
