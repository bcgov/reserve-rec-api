const { runQuery } = require('../../layers/awsUtils/dynamodb');
const { logger, sendMessage } = require('/opt/base');
const { batchWriteData, AUDIT_TABLE_NAME, marshall, unmarshall, USER_ID_PARTITION, runQuery } = require('/opt/dynamodb');
...
async function sendToAllConnections() {
  ...
  try {
    ...
    for (const record of result.items) {
      const connectionId = record.sk.S;
      sendMessage(connectionId, OPENSEARCH_DOMAIN_NAME, STAGE_NAME, message);
    }
  } catch (error) {
