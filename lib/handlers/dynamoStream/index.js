...
const { batchWriteData, AUDIT_TABLE_NAME, marshall, unmarshall, USER_ID_PARTITION, queryData } = require('/opt/dynamodb');
...
async function sendToAllConnections() {
  ...
  try {
    const result = await queryData(item);
    const message = "Testing message";
    for (const record of result.Items) {
      const connectionId = record.sk.S;
      await sendMessage(connectionId, OPENSEARCH_DOMAIN_NAME, STAGE_NAME, message);
    }
  } catch (error) {
