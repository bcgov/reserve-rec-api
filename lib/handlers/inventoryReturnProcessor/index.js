...
async function validateInventoryReturn(inventory, booking) {
  // UTC on purpose: Pacific is fixed at UTC-7, so the UTC day rolls at 17:00
  // local. A same-day pass cancelled after 5pm is deliberately not returned.
  const now = getNow();
...
function validateInventoryPoolReturn(inventoryPoolItem, dateStr) {
  ...
  // UTC on purpose — see validateInventoryReturn: same-day returns stop at 17:00 Pacific.
  const now = getNow();
