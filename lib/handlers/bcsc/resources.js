...
function bcscSetup(scope, props) {
  ...
  const bcscGetFunction = new NodejsFunction(scope, 'BcscGetFunction', {
    ...
    layers: props.layers,
    timeout: cdk.Duration.seconds(30), // Add timeout - 30 seconds should be enough
    memorySize: 256, // Also consider increasing memory for RSA key generation
  });
  ...
  const bcscPostFunction = new NodejsFunction(scope, 'BcscPostFunction', {
    ...
    environment: props.env,
    layers: props.layers,
    timeout: cdk.Duration.seconds(30), // Add timeout for POST as well
