...
function transactionsSetup(scope, props) {
  ...
  const transactionsGet = new NodejsFunction(scope, 'TransactionsGet', {
    code: lambda.Code.fromAsset('lib/handlers/transactions/GET'),
  ...
  const transactionsPost = new NodejsFunction(scope, 'TransactionsPost', {
    code: lambda.Code.fromAsset('lib/handlers/transactions/POST'),
    handler: 'index.handler',
    runtime: lambda.Runtime.NODEJS_20_X,
    environment: props.env,
    layers: props.layers,
  ...
  // PUT /transactions
  const transactionsPut = new NodejsFunction(scope, 'TransactionsPut', {
    code: lambda.Code.fromAsset('lib/handlers/transactions/PUT'),
    handler: 'index.handler',
    runtime: lambda.Runtime.NODEJS_20_X,
    environment: props.env,
    layers: props.layers,
  ...
  transactionsResource.addMethod('PUT', new apigateway.LambdaIntegration(transactionsPut));
  ...
  transactionsPut.addToRolePolicy(dynamodbPolicy);
  ...
  return {
    ...
    transactionsPut: transactionsPut,
  };
