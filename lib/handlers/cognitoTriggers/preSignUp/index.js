...
// The domain is already on the list; the local part is logged only where it is
// the thing that matched (see refuse).
...
function refuse(event, email, reason) {
  const fields = { reason, ...signupFields(event, email) };
  // A pattern matches on the shape of the local part, so that is the evidence.
  // An address or domain refusal is already named by the entry that caught it.
  if (reason === 'pattern') fields.localPart = canonicalizeEmail(email)?.split('@')[0];
