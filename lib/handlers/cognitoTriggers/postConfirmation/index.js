 ...
 *
 * Refusal is gated by EMAIL_CHANGE_REFUSE.
 *
 * @param {string} sub
...
async function checkEmailChange(sub, source, newEmail) {
  ...
  if (source === 'bcsc') return;

  let reason = null;
  try {
    reason = refusalReason(newEmail, await loadBlocklist(process.env.BLOCKLIST_TABLE_NAME));
  } catch (err) {
  ...
  if (process.env.EMAIL_CHANGE_REFUSE === 'true') throw new EmailChangeRefused();
}

async function reviewFields(existingUser, { isBcscUser, emailChanged, sub, email, now }) {
  const kept = Object.fromEntries(Object.entries(existingUser || {}).filter(([key]) => key.startsWith('review')));
  if (!isBcscUser || (kept.reviewFlaggedAt && !emailChanged)) return kept;

  let reason;
  try {
    reason = refusalReason(email, await loadBlocklist(process.env.BLOCKLIST_TABLE_NAME));
  } catch (err) {
    logger.error('PreTokenGeneration review check failed open', { error: err.message });
    return kept;
  }
  if (!reason) return kept;

  logger.info('event=account_flagged', { sub, reason });
  return { ...kept, reviewFlag: true, reviewReason: reason, reviewFlaggedAt: now };
}
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const emailChanged = Boolean(existingUser && typeof existingUser.email === 'string' && existingUser.email
    ...
    const review = await reviewFields(existingUser, { isBcscUser, emailChanged, sub, email: userAttributes.email, now });
