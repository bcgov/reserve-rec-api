 ...
 * 
 * The URL format is: https://{altereDomain}/verify/{bookingId}/{hash}
...
function generateQRURL(bookingId, secretKey = QR_SECRET_KEY) {
  ...
  // Construct verification URL using the admin frontend altered domain (vanity url), so scanning
  ...
  const alteredDomain = ADMIN_ALTERED_FRONTEND_DOMAIN || 'localhost:4300'; // Fallback for tests
