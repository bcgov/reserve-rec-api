...
const ADMIN_FRONTEND_DOMAIN = process.env.ADMIN_FRONTEND_DOMAIN;
const ADMIN_ALTERED_FRONTEND_DOMAIN = process.env.ADMIN_ALTERED_FRONTEND_DOMAIN;
...
function validateEnvironment() {
  ...
  if (!ADMIN_FRONTEND_DOMAIN) {
    missing.push('ADMIN_FRONTEND_DOMAIN');
  }

  if (!ADMIN_ALTERED_FRONTEND_DOMAIN) {
    missing.push('ADMIN_ALTERED_FRONTEND_DOMAIN');
  }
  ...
  logger.debug('QR code environment validated', {
    hasSecretKey: true,
    domain: ADMIN_FRONTEND_DOMAIN,
    alteredDomain: ADMIN_ALTERED_FRONTEND_DOMAIN
  });
}
...
function generateQRURL(bookingId, secretKey = QR_SECRET_KEY) {
  ...
  // Construct verification URL using the admin frontend domain, so scanning
  // the QR code (e.g. with a phone's native camera) opens Admin's Sales
  // screen instead of the Public site (bcgov/reserve-rec-admin#335)
  const domain = ADMIN_FRONTEND_DOMAIN || 'localhost:4300'; // Fallback for tests
  const alteredDomain = ADMIN_ALTERED_FRONTEND_DOMAIN || 'localhost:4300';
  const verificationUrl = `https://${alteredDomain}/verify/${bookingId}/${hash}`;
