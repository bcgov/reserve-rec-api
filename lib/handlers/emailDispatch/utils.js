...
/**
 * Redact an email address for logging — keep the domain so we can spot
 * deliverability issues by provider, but drop the local part so PII doesn't
 * end up in CloudWatch.
 *
 * "alice.smith@gmail.com" -> "***@gmail.com"
 */
...
async function sendEmailMessage(payload) {
  ...
  try {
    ...
    return {
      ...
      messageType: payload.messageType,
      templateName: payload.templateName,
      // Domain only — the full address must not appear in any caller's logs.
      recipientDomain: redactEmail(payload.recipientEmail),
    };
