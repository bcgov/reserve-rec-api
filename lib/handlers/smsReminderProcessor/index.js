...
const { normalizePhoneNumber } = require('/opt/phone');

let smsClient;

// Client errors that will fail the same way on every retry. Log and drop
// these so one bad message does not loop until the queue gives up
// (bcgov/reserve-rec-public#862). Throttling, quota and server errors are
// left to throw, so SQS retries them.
...
exports.handler = async (event) => {
  ...
  for (const record of records) {
    ...
    try {
      ...
      logger.error('SMS reminder dropped: non-retryable error', {
        bookingId: payload?.bookingId || null,
        phoneNumber,
