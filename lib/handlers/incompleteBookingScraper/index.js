...
const { getExpiredBookings, deleteBookingHoldMarker } = require("../../../src/handlers/bookings/methods");
...
exports.handler = async (event, context) => {
  ...
  for (const projectedBooking of expiredBookings.items) {
    ...
    try {
      ...
      // Release the hold marker so the slot frees up for re-booking. Best-effort;
      // idempotent Delete, so a missing marker is fine.
      try {
        await batchTransactData([deleteBookingHoldMarker(booking)]);
      } catch (markerError) {
        logger.warn(`Failed to delete booking-hold marker for ${booking.bookingId}: ${markerError?.message}`);
      }
    } catch (error) {
