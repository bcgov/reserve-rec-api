...
class PublicBookingsNestedStack extends NestedStack {
  ...
  constructor(scope, id, props) {
    ...
    const eventAlarms = props.eventAlarms || {};
    addEventMetrics(postFn, metricNamespace,
      ['hold_created', 'hold_failed', 'hold_conflict', 'hold_refused_has_booking', 'hold_refused_has_hold',
        'booking_refused_unverified_email'], eventAlarms);
    ...
    addEventMetrics(completeFn, metricNamespace,
      ['booking_completed', 'booking_complete_failed', 'booking_refused_unverified_email'], eventAlarms);
    ...
    // Opt-in per environment so one can run ahead of the others. Without the
    // queue url the handler logs a failure and the cancellation still succeeds.
    if (props.cancelEmailEnabled) {
      const cancelFn = this.publicBookingsConstruct.bookingsCancelPostFunction;
      if (props.emailQueueUrl) {
        cancelFn.addEnvironment('EMAIL_QUEUE_URL', props.emailQueueUrl);
      }
      if (props.emailQueueArn) {
