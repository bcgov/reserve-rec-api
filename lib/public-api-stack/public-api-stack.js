 ...
 * Public API Stack
 * 
 * This stack defines the Public API Gateway and associated Lambda functions 
 * or the Reserve Rec APIs.
 * 
 * For local development with SAM, the stack uses flat constructs instead of nested
 * stacks to avoid issues with referencing resources across nested stack boundaries
 * in a local environment. In an AWS deployment, it uses nested stacks to better
 * organize resources and reduce the number of resources in the root stack, it also
 * includes feature flags for selective deployment of different API areas.
 *
 */
...
const { addOverviewDashboard, addBookingsDashboard } = require('../helpers/dashboards');
...
const { PublicInventoryPoolsConstruct }          = require('../../src/handlers/inventoryPools/constructs');
...
const { PublicInventoryPoolsNestedStack }        = require('./public-inventory-pools-nested-stack/public-inventory-pools-nested-stack');
...
const { PublicCollectionsNestedStack }           = require('./public-collections-nested-stack/public-collections-nested-stack');
...
const defaults = {
  ...
  constructs: {
    ...
    publicInventoryPoolsConstruct:    { name: "PublicInventoryPoolsConstruct" },
    activitiesConstruct:              { name: "ActivitiesConstruct" },
    geozonesConstruct:                { name: "GeozonesConstruct" },
    publicTransactionsConstruct:      { name: "PublicTransactionsConstruct" },
    collectionsConstruct:             { name: "PublicCollectionsConstruct" },
    publicConfigLambdas:              { name: "PublicConfigLambdas" },
    ...
    publicSearchLambda:               { name: "PublicSearchLambda" },
    publicTransactionsNestedStack:    { name: "PublicTransactionsNestedStack" },
    bookingsNestedStack:              { name: "PublicBookingsNestedStack" },
    collectionsNestedStack:           { name: "PublicCollectionsNestedStack" },
    ...
    overviewDashboard:                { name: "OverviewDashboard" },
    bookingsDashboard:                { name: "BookingsDashboard" },
    productDatesNestedStack:          { name: "PublicProductDatesNestedStack" },
    productsNestedStack:              { name: "PublicProductsNestedStack" },
    inventoryPoolsNestedStack:        { name: "PublicInventoryPoolsNestedStack" },
  ...
  config: {
    ...
    enablePublicTransactions: process.env.ENABLE_PUBLIC_TRANSACTIONS !== 'false',
    ...
    enableCollections: process.env.ENABLE_COLLECTIONS !== 'false',
    enableInventoryPools: process.env.ENABLE_INVENTORY_POOLS !== 'false',
  },
  secrets: {
    merchantId: { name: "merchantId", },
    hashKey: { name: "hashKey", },
    paymentsApiPasscode: { name: "paymentsApiPasscode", },
    worldlineWebhookSecret: { name: "worldlineWebhookSecret", },
    qrSecretKey: { name: "qrSecretKey", },
  },
};
...
class PublicApiStack extends BaseStack {
  constructor(scope, primer) {
    ...
    const adminAlteredFrontendDomain  = scope.resolveAdminAlteredDomainName(this);
    // Absent means off, so an environment enables it by setting the key.
    const cancelEmailEnabled          = ['true', true].includes(this.getConfigValue('enableCancellationEmail'));
    ...
    if (this.getConfigValue('enableSearch')) {
      ...
      if (isLocal) {
        this.publicSearchConstruct = new PublicSearchLambda(
          this,
          this.getConstructId('publicSearchLambda'),
          searchProps,
        );
      } else {
        ...
        this.publicSearchConstruct = this.publicSearchNestedStack.publicSearchConstruct;
      }
    }

    // Collections Lambdas
    if (this.getConfigValue('enableCollections')) {
      ...
      if (isLocal) {
        this.collectionsConstruct = new PublicCollectionsConstruct(
          this,
          this.getConstructId('collectionsConstruct'),
          collectionsProps,
        );
      } else {
        this.collectionsNestedStack = new PublicCollectionsNestedStack(
          this,
          this.getConstructId('collectionsNestedStack'),
          collectionsProps,
        );
        this.collectionsConstruct = this.collectionsNestedStack.collectionsConstruct;
      }
    }
    ...
    // Public inventory pool endpoint, but only used for products
    if (this.getConfigValue('enableProducts')) {
      const inventoryPoolsProps = {
        environment: {
          LOG_LEVEL: this.getConfigValue('logLevel'),
          REFERENCE_DATA_TABLE_NAME: referenceDataTableName,
        },
      ...
      if (isLocal) {
        this.publicInventoryPoolsConstruct = new PublicInventoryPoolsConstruct(
          this,
          this.getConstructId('publicInventoryPoolsConstruct'),
          inventoryPoolsProps,
        );
      } else {
        // For AWS deployments, use the nested stack to reduce resource count
        // in the root stack
        this.inventoryPoolsNestedStack = new PublicInventoryPoolsNestedStack(
          this,
          this.getConstructId('inventoryPoolsNestedStack'),
          inventoryPoolsProps,
        );
        this.publicInventoryPoolsConstruct = this.inventoryPoolsNestedStack.publicInventoryPoolsConstruct;
      }
    }
    ...
    if (this.getConfigValue('enableBookings')) {
      const bookingsProps = {
        environment: {
          ...
          ADMIN_ALTERED_FRONTEND_DOMAIN: adminAlteredFrontendDomain,
          // Base for user-facing links in booking emails (view/cancel buttons —
          // see bookings/methods.js). Host + path prefix, no scheme; behind the
          // front door this is e.g. dev-reserve.bcparks.ca/dayuse.
        ...
        handlerPrefix: 'public',
        kmsKey: kmsKey,
        eventAlarms: this.getConfigValue('eventAlarms'),
        ...
        cancelEmailEnabled: cancelEmailEnabled,
        waitingRoomTable: scope.getWaitingRoomTable(),
    ...
    // Public Transactions Lambdas
    if (this.getConfigValue('enablePublicTransactions')) {
      const transactionsProps = {
        environment: {
          LOG_LEVEL: this.getConfigValue('logLevel'),
          TRANSACTIONAL_DATA_TABLE_NAME: transactionalDataTableName,
          MERCHANT_ID_SECRET: `/reserveRecApi/${this.getDeploymentName()}/publicApiStack/merchantId`,
          HASH_KEY_SECRET: `/reserveRecApi/${this.getDeploymentName()}/publicApiStack/hashKey`,
          WORLDLINE_PAYMENT_API_URL: 'https://api.na.bambora.com/v1/payments',
          PAYMENTS_API_SECRET: `/reserveRecApi/${this.getDeploymentName()}/publicApiStack/paymentsApiPasscode`
        },
        layers: [baseLayer, awsUtilsLayer],
        // For local/SAM: pass the object. For nested: pass the imported API.
        api: isLocal ? this.publicApiCoreNestedStack.publicApi : null,
      ...
      // For local, use the flat construct for SAM compatibility.
      if (isLocal) {
        this.publicTransactionsConstruct = new PublicTransactionsConstruct(
          this,
          this.getConstructId('publicTransactionsConstruct'),
          transactionsProps,
        );
      } else {
        // For AWS deployments, use the nested stack to reduce resource count
        // in the root stack
        this.publicTransactionsNestedStack = new PublicTransactionsNestedStack(
          this,
          this.getConstructId('publicTransactionsNestedStack'),
          transactionsProps,
        );
        this.publicTransactionsConstruct = this.publicTransactionsNestedStack.publicTransactionsConstruct;
      }
    }
    ...
    const nestedStacks = [
      ...
      isLocal ? null : this.publicTransactionsNestedStack,
      isLocal ? null : this.productDatesNestedStack,
      isLocal ? null : this.inventoryPoolsNestedStack,
    ...
    // One structured line per request, which is what dashboards and alarms
    // read. Execution logging alone gives neither: it is per-method tracing,
    // not a queryable stream.
    const publicAccessLogs = new logs.LogGroup(this, 'PublicApiAccessLogs', {
      logGroupName: `/aws/apigateway/${this.getAppName()}-${this.getDeploymentName()}-public-access`,
    ...
    const stage = new apigw.Stage(this, 'PublicApiStage', {
      ...
      // Payload tracing writes full request and response bodies to CloudWatch,
      // which in production means personal data at two-year retention. The
      // access log below carries what is actually useful without the bodies.
      dataTraceEnabled: false,
      metricsEnabled: true,
      accessLogDestination: new apigw.LogGroupLogDestination(publicAccessLogs),
      accessLogFormat: apigw.AccessLogFormat.custom(JSON.stringify({
        ...
        // Set by the authorizer; 'guest' when the caller is unauthenticated.
        userId: apigw.AccessLogField.contextAuthorizer('userId'),
        requestTime: apigw.AccessLogField.contextRequestTime(),
        httpMethod: apigw.AccessLogField.contextHttpMethod(),
        // resourcePath keeps the template (/facilities/{collectionId}) while
        // path keeps the real one — the first groups, the second identifies
        // which facility, which is what busy-time analysis needs.
    ...
    // The operations pages that read this API (lib/helpers/dashboards.js).
    // The overview lists both stacks' alarms by name; the identity stack's
    // thresholds are read from its config, not referenced.
    if (!isLocal) {
      const namespace = eventMetricNamespace(this.getDeploymentName());
      const api = { name: this.publicApiCoreNestedStack.publicApi.restApiName, stage: stage.stageName };
      const alarmNames = [
        ...Object.keys(this.getConfigValue('eventAlarms') || {}),
        ...Object.keys(scope.getStackByKey('publicIdentityStack')?.config?.eventAlarms || {}),
      ];
      addOverviewDashboard(this, this.getConstructId('overviewDashboard'), namespace, api,
        `${this.getAppName()}-${this.getDeploymentName()}-`, alarmNames,
        scope.getStackByKey('publicIdentityStack').getConstructId('preSignUpFunction'));
      addBookingsDashboard(this, this.getConstructId('bookingsDashboard'), namespace, api);
    }
