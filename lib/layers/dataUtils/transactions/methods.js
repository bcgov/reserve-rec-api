...
async function updateTransaction(clientTransactionId, bookingId, sessionId, params) {
  logger.debug("updating transaction: ", params);
  try {
    ...
    // Confirm that the transaction is completed
    const response = await queryTransaction(transactionRecord);

    logger.debug("query transaction response: ", response);

    if (response.messageText == "Approved") {
      params.status = "paid";
    } else if (response.messageText == "DECLINE") {
...
async function queryTransaction(transaction) {
  ...
  const query = new URLSearchParams({
    requestType: "BACKEND",
    merchantid: MERCHANT_ID, // different from merchant_id, because...
    trnType: "Q",
    trnOrderNumber: transaction.clientTransactionId,
  });

  const queryUrl = createUrlWithHash(query, url);

  // Get the transaction data from Worldline
  return await pollUntilAnswered(queryUrl);
}
...
function pollUntilAnswered(queryUrl, interval = 3000, timeout = 30000) {
  const start = Date.now();

  // Make a promise to poll Worldline
  return new Promise((resolve, reject) => {
    const poll = async () => {
      try {
        const response = await axios.get(queryUrl);
        logger.info("Worldline response.data:", response.data);
        ...
        logger.info("Got a polling response from Worldline");
        if (!response.data || typeof response.data !== "string") {
          return reject(new Error("Invalid response format"));
        }

        const dataObj = Object.fromEntries(new URLSearchParams(response.data));
        logger.debug("dataObj from Worldline: ", dataObj);

        // If we get Approved or Declined, then we resolve the poll
        if (
          dataObj.messageText == "Approved" ||
          dataObj.messageText == "DECLINE"
        ) {
          return resolve(dataObj);
        }

        // Otherwise, we attempt to query for the transaction again
        ...
        reject(new Error("Error querying transaction"));
      }
    };
