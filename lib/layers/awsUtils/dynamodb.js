...
async function incrementCounter(pk, collectionType = []) {
  ...
  try {
    ...
    // Throw an error immediately if the counter is missing for a transaction,
    // as that's bad and not easily replaceable like other counter items.
    if (counterItemMissing && pk === "transaction") {
      throw new Exception('transaction counter item missing', { code: 400 });
    }
...
async function getByGSI(gsiName, gsiValue, tableName = TABLE_NAME, indexName) {
  logger.info(`getItem by index: { ${gsiName}: ${gsiValue}, indexName: ${indexName} }`);

  const params = {
    TableName: tableName,
    IndexName: indexName,
    KeyConditionExpression: `#${gsiName} = :${gsiName}`,
    ExpressionAttributeValues: {
