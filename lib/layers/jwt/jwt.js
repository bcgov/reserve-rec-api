...
class JWTManager {
  ...
  // Get the RSA key components properly
  const key = new NodeRSA(this.keyPair.public);
  const components = key.exportKey('components-public');
  
  // The 'e' should always be AQAB for standard RSA keys
  const e = "AQAB";
  
  // Convert the modulus (n) to proper base64url
  ...
  // Match the exact structure and order from your example
  const jwksKey = {
    n: n,
    e: e,
    kty: "RSA",
