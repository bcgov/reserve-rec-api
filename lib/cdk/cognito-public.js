...
function cognitoPublicSetup(scope, props) {
    ...
    const userPoolDomain = new cognito.UserPoolDomain(scope, 'PublicUserPoolDomain', {
        cognitoDomain: {
            domainPrefix: `${props.env.project}-public-${props.env.environmentNameName}`
        },
    ...
    new cognito.CfnUserPoolUICustomizationAttachment(scope, 'PublicCognitoUICustomization', {
      ...
      // Outputs
    new cdk.CfnOutput(scope, `PublicUserPoolId-${props.env.environmentName}`, {
        value: publicUserPool.userPoolId,
        description: 'Public User Pool Id Reference',
        exportName: `PublicUserPoolId-${props.env.environmentName}`,
    });

    new cdk.CfnOutput(scope, `PublicUserPoolClientId-${props.env.environmentName}`, {
        value: publicUserPoolClient.userPoolClientId,
        description: 'Public User Pool Client Id Reference',
        exportName: `PublicUserPoolClientId-${props.env.environmentName}`,
    });

    new cdk.CfnOutput(scope, `PublicIdentityPoolId-${props.env.environmentName}`, {
        value: publicIdentityPool.ref,
        description: 'Public Identity Pool Id Reference',
        exportName: `PublicIdentityPoolId-${props.env.environmentName}`,
    });

    new cdk.CfnOutput(scope, `PublicUserPoolDomainUrl-${props.env.environmentName}`, {
        ...
        exportName: `PublicUserPoolDomainUrl-${props.env.environmentName}`,
    });
