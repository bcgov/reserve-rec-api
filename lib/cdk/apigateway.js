...
function apigwSetup(scope, props) {
  ...
  new cdk.CfnOutput(scope, 'ReserveRecApiGatewayId', {
    ...
    exportName: `ReserveRecApiGatewayId-${props.env.environmentName}`,
  });
  ...
  new cdk.CfnOutput(scope, 'ReserveRecApiRootResourceId', {
    ...
    exportName: `ReserveRecApiRootResourceId-${props.env.environmentName}`,
  });

  new cdk.CfnOutput(scope, `ApiLocation-${props.env.environmentName}`, {
    value: `https://${api.restApiId}.execute-api.${scope.region}.amazonaws.com/`,
    description: 'Reserve Rec API URL',
    exportName: `ApiLocation-${props.env.environmentName}`,
  });

  new cdk.CfnOutput(scope, `ApiPath-${props.env.environmentName}`, {
    value: api.deploymentStage.stageName,
    description: 'Reserve Rec API Stage',
    exportName: `ApiPath-${props.env.environmentName}`,
  });
