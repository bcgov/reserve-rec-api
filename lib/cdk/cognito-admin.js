...
function cognitoAdminSetup(scope, props) {
  ...
  const userPoolDomain = new cognito.UserPoolDomain(scope, 'AdminUserPoolDomain', {
    cognitoDomain: {
      domainPrefix: `${props.env.cognitoDomainPrefix}-${props.env.environmentName}`
    },
    userPool: userPool
  });
  ...
  // Outputs
  new cdk.CfnOutput(scope, `AdminUserPoolId-${props.env.environmentName}`, {
    value: userPool.userPoolId,
    description: 'Admin User Pool Id Reference',
    exportName: `AdminUserPoolId-${props.env.environmentName}`,
  });

  new cdk.CfnOutput(scope, `AdminUserPoolClientId-${props.env.environmentName}`, {
    value: userPoolClient.userPoolClientId,
    description: 'Admin User Pool Client Id Reference',
    exportName: `AdminUserPoolClientId-${props.env.environmentName}`,
  });

  new cdk.CfnOutput(scope, `AdminIdentityPoolId-${props.env.environmentName}`, {
    value: identityPool.ref,
    description: 'Admin Identity Pool Id Reference',
    exportName: `AdminIdentityPoolId-${props.env.environmentName}`,
  });

  new cdk.CfnOutput(scope, `AdminUserPoolDomainUrl-${props.env.environmentName}`, {
    ...
    exportName: `AdminUserPoolDomainUrl-${props.env.environmentName}`,
  });
