...
function dynamodbSetup(scope, props) {
  ...
  // If the environment is dev, use the existing table
  // This is a temporary fix until the main table is managed by the CDK stack
  if (props.env.environmentName === 'dev') {
    mainTable = dynamodb.Table.fromTableArn(scope, 'MainTable', 'arn:aws:dynamodb:ca-central-1:637423314715:table/reserve-rec-main');
  } else {
    // If the environment is not dev, create a new table
    // Main Reserve Rec Data Table
