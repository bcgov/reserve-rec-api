...
function dynamodbSetup(scope, props) {
  ...
  // Determine stack naming strategy
  const stackSuffix = process.env.STACK_NAME || null;
  const stackName = stackSuffix || 'ReserveRecCdkStack';
  
  // Determine deployment strategy based on environment and parameter availability
  const isFirstDeployment = process.env.FORCE_CREATE_TABLES === 'true' || checkIfFirstDeployment(stackName);
...
/**
 * Helper function to detect if this is a first deployment
 */
function checkIfFirstDeployment(stackName) {  
  // Check if stack name is something new
  if (stackName && stackName !== 'ReserveRecCdkStack') {
    console.log(`Non-standard stack name detected: ${stackName} - assuming first deployment`);
    return true;
  }
...
function getTableArnFromParameterStore(scope, tableType, stackName) {
  try {
    // Parameter Store path strategy:
    // - For ReserveRecCdkStack (default): /reserve-rec/main/tables/{tableType}/arn
    // - For custom stacks (e.g. test): /reserve-rec/{stackName}/tables/{tableType}/arn
    const parameterPath = stackName === 'ReserveRecCdkStack' 
      ? `/reserve-rec/main/tables/${tableType}/arn`
      : `/reserve-rec/${stackName}/tables/${tableType}/arn`;
