...
function dynamodbSetup(scope, props) {
  ...
  // Try to get table ARNs from Parameter Store first
  try {
    ...
    console.log('main/audit/pubsub tables retrieved', mainTableArn, auditTableArn, pubsubTableArn);
    ...
    if (mainTableArn && auditTableArn && pubsubTableArn) {
      ...
      console.log('Using existing tables from Parameter Store:', mainTable?.tableName, auditTable?.tableName, pubsubTable?.tableName);
